    ...
    internal fun FirAnnotatedDeclaration.loadExperimentalities(
        ...
        if (this is FirCallableMemberDeclaration<*>) {
            ...
            if (this !is FirConstructor) {
                result.addAll(returnTypeRef.coneType.loadExperimentalities(context, visited))
                result.addAll(receiverTypeRef?.coneType.loadExperimentalities(context, visited))
                if (this is FirSimpleFunction) {
                    valueParameters.forEach {
                        result.addAll(it.returnTypeRef.coneType.loadExperimentalities(context, visited))
                    }
                }
            }
            if (parentClass != null) {
